...
class ManagePlanController extends Controller
{
    ...
    public function planStore(Request $request)
    {
        $this->validate($request, [
            ...
            'ref_com' => 'required|numeric|min:0|max:100', // Cap at 100%
            'indir_com' => 'required|numeric|min:0|max:100', // New field
        ...
        $plan->indir_com = $request->indir_com; // New field
        ...
        if ($request->default) {
            \DB::table('plans')->where('id', '<>', $plan->id)->update(['is_default' => 0]);
        }
    ...
    public function planUpdate(Request $request)
    {
        ...
        $plan = Plan::findOrFail($request->id); // Use findOrFail
        $plan->name = $request->name;
        $plan->min_price = $request->minprice;
        $plan->max_price = $request->maxprice;
        $plan->ref_com = $request->ref_com;
        ...
        if ($request->default) {
            \DB::table('plans')->where('id', '<>', $request->id)->update(['is_default' => 0]);
        }
